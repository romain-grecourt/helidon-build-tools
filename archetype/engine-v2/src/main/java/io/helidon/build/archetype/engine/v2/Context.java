...
public final class Context {
    ...
    public Scope pushScope(Scope scope) {
        if (this.scope != scope.parent) {
            throw new IllegalArgumentException(String.format(
                    "Invalid scope: current=%s, given=%s", this.scope.key(true), scope.key(true)));
        }
        this.scope = scope;
        return scope;
    }
    ...
    public static class Scope {
        ...
        static final Scope EMPTY = new Scope() {
            @Override
            public ScopeValue<?> value(Value<?> newValue, ValueKind kind) {
        ...
        public ScopeValue<?> value(Value<?> newValue, ValueKind kind) {
            if (value.isWritable()) {
                value = new ScopeValue<>(this, newValue, kind);
            }
        ...
        public Map<String, ScopeValue<?>> values() {
            Map<String, ScopeValue<?>> values = new HashMap<>();
            visit(node -> {
                ScopeValue<?> value = node.value();
                if (value.isPresent()) {
    ...
    public static final class ScopeValue<T> implements Value<T> {
        ...
        ScopeValue(Scope scope, Value<T> delegate, ValueKind kind) {
