...
import static io.helidon.build.archetype.engine.v2.ScriptCompiler.Features.VALIDATE;
...
import static io.helidon.build.archetype.engine.v2.ScriptCompiler.Features.NO_TRANSIENT;
import static io.helidon.build.archetype.engine.v2.ScriptCompiler.Features.NO_OUTPUT;
...
class ScriptCompilerTest {
    ...
    void testEmptyScript() {
        Path outputDir = compile("compiler/empty-script", "main.xml", NO_OUTPUT);
    ...
    @Test
    void testFiltering1() {
        Path outputDir = compile("compiler/filtering1", "main.xml", NO_OUTPUT, NO_TRANSIENT);
    ...
    @Test
    void testInlined4() {
        Path outputDir = compile("compiler/inlined4", "main.xml");
    ...
    @Test
    void testInput3() {
        Path outputDir = compile("compiler/input3", "main.xml");
    ...
    @Test
    void testVariables1() {
        Path outputDir = compile("compiler/variables1", "main.xml");
    ...
    void testOptionalInputWithoutDefault() {
        try {
            compile("compiler/validate", "optional-input-no-default.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
            assertThat(it.next(), containsString(INPUT_OPTIONAL_NO_DEFAULT));
    ...
    void testOptionalStepWithNonOptionalInputs() {
        try {
            compile("compiler/validate", "optional-step-required-input.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
            assertThat(it.next(), containsString(STEP_DECLARED_OPTIONAL));
            assertThat(it.hasNext(), is(false));
        }
    }
    ...
    void testRequiredStepWithOnlyOptionalInputs() {
        try {
            compile("compiler/validate", "required-step-optional-input.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
            assertThat(it.next(), containsString(STEP_NOT_DECLARED_OPTIONAL));
            assertThat(it.hasNext(), is(false));
        }
    }
    ...
    void testRequiredStepWithinOptionalStep() {
        try {
            compile("compiler/validate", "required-step-within-optional-step.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
    ...
    void testStepWithNoInputs() {
        try {
            compile("compiler/validate", "step-with-no-inputs.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
            assertThat(it.next(), containsString(STEP_NO_INPUT));
            assertThat(it.hasNext(), is(false));
        }
    }
    ...
    void testOptionalStepWithinRequiredStep() {
        compile("compiler/validate", "optional-step-within-required-step.xml", VALIDATE);
    }

    @Test
    void testUnresolvedPreset() {
        try {
            compile("compiler/validate", "unresolved-preset.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
            assertThat(it.next(), containsString(PRESET_UNRESOLVED));
            assertThat(it.hasNext(), is(false));
        }
    }
    ...
    void testPresetTypeMismatch() {
        try {
            compile("compiler/validate", "preset-type-mismatch.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
            assertThat(it.next(), containsString(PRESET_TYPE_MISMATCH));
            assertThat(it.hasNext(), is(false));
        }
    }
    ...
    void testExpressionWithIncompatibleOperators() {
        try {
            compile("compiler/validate", "expression-incompatible-operators.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
            assertThat(it.next(), containsString(EXPR_INCOMPATIBLE_OPERATOR));
            assertThat(it.hasNext(), is(true));
            assertThat(it.next(), containsString(EXPR_INCOMPATIBLE_OPERATOR));
            assertThat(it.hasNext(), is(true));
            assertThat(it.next(), containsString(EXPR_INCOMPATIBLE_OPERATOR));
    ...
    void testExpressionWithUnresolvedVariable1() {
        try {
            compile("compiler/validate", "expression-unresolved-variable1.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
            assertThat(it.next(), containsString(EXPR_UNRESOLVED_VARIABLE));
            assertThat(it.hasNext(), is(false));
        }
    }
    ...
    void testExpressionWithUnresolvedVariable2() {
        try {
            compile("compiler/validate", "expression-unresolved-variable2.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
    ...
    void testExpressionWithUnresolvedVariable3() {
        compile("compiler/validate", "expression-unresolved-variable3.xml", VALIDATE);
    }

    @Test
    void testExpressionWithUnresolvedVariable4() {
        compile("compiler/validate", "expression-unresolved-variable4.xml", VALIDATE);
    }

    @Test
    void testExpressionWithTypeMismatch1() {
        try {
            compile("compiler/validate", "expression-type-mismatch1.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
            assertThat(it.next(), containsString(EXPR_EVAL_ERROR));
            assertThat(it.hasNext(), is(false));
        }
    }
    ...
    void testExpressionWithTypeMismatch2() {
        try {
            compile("compiler/validate", "expression-type-mismatch2.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
    ...
    void testInputAlreadyDeclared1() {
        try {
            compile("compiler/validate", "input-already-declared1.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
            assertThat(it.next(), containsString(INPUT_ALREADY_DECLARED));
            assertThat(it.hasNext(), is(false));
        }
    }
    ...
    void testInputAlreadyDeclared2() {
        try {
            compile("compiler/validate", "input-already-declared2.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
    ...
    void testInputTypeMismatch() {
        try {
            compile("compiler/validate", "input-type-mismatch.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
            assertThat(it.next(), containsString(INPUT_TYPE_MISMATCH));
            assertThat(it.hasNext(), is(false));
        }
    }
    ...
    void testInputNotInStep() {
        try {
            compile("compiler/validate", "input-not-in-step.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
            assertThat(it.next(), containsString(INPUT_NOT_IN_STEP));
            assertThat(it.hasNext(), is(false));
        }
    }
    ...
    void testOptionValueAlreadyDeclared() {
        try {
            compile("compiler/validate", "option-value-already-declared.xml", VALIDATE);
            fail("An exception should have been thrown");
        } catch (ValidationErrors ex) {
            Iterator<String> it = ex.errors.iterator();
            assertThat(it.hasNext(), is(true));
            assertThat(it.next(), containsString(OPTION_VALUE_ALREADY_DECLARED));
            assertThat(it.hasNext(), is(false));
        }
    }

    static Path compile(String path, String entrypoint, ScriptCompiler.Features... features) {
        ...
        try (FileSystem fs = VirtualFileSystem.create(testClasses)) {
            ...
            ScriptCompiler compiler = new ScriptCompiler(new ScriptCompiler.Options()
                    .source(cwd.resolve(entrypoint))
                    .cwd(cwd)
                    .outputDir(outputDir)
                    .features(features));
            if (!compiler.compile()) {
