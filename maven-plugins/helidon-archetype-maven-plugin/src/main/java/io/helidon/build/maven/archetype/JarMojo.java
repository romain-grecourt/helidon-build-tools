...
import static io.helidon.build.maven.archetype.ScriptCompilerExt.Features.VALIDATE_REGEX;
import static io.helidon.build.maven.archetype.ScriptCompilerExt.Features.VALIDATE_SCHEMA;
        ...
        try {
            ...
            try (FileSystem fs = VirtualFileSystem.create(sourceDirectory.toPath())) {
                ...
                ScriptCompiler compiler = new ScriptCompiler(new ScriptCompiler.Options()
                        .source(entrypoint(cwd))
                        .cwd(cwd)
                        .outputDir(archetypeDir)
                        .features(VALIDATE, VALIDATE_REGEX, VALIDATE_SCHEMA));
                if (!compiler.compile()) {
