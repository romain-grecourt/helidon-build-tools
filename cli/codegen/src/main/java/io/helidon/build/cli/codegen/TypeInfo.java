...
interface TypeInfo extends Comparable<TypeInfo> {
    ...
    /**
     * Get a {@link TypeInfo} that describes the type qualified name.
     *
    ...
    /**
     * Get a {@link TypeInfo} that is composed by a type and given type parameters.
     *
    ...
    default boolean is(Class<?> aClass) {
        return qualifiedName().equals(aClass.getTypeName());
    }
    ...
    /**
     * Test if this type is compatible with the given class (equal, interface, superclass).
     *
     * @param aClass class to be compared
     * @return {@code true} if equal, {@code false} otherwise
     */
    default boolean isCompatible(Class<?> aClass) {
        if (!qualifiedName().equals(aClass.getTypeName())) {
            if (Arrays.stream(interfaces())
                      .anyMatch(t -> t.qualifiedName().equals(aClass.getTypeName()))) {
                return true;
            }
            TypeInfo type = this.superClass().orElse(null);
            while (type != null) {
                if (type.isCompatible(aClass)) {
                    return true;
                }
     ...
     *
     * @param interfaceType interface to look for
     * @return {@code true} if found, {@code} false otherwise
     */
    default boolean hasInterface(Class<?> interfaceType) {
        boolean found = false;
        TypeInfo info = this;
        while (info != null) {
            if (stream(info.interfaces()).anyMatch(i -> i.isCompatible(interfaceType))) {
                found = true;
                break;
            }
            info = info.superClass().orElse(null);
        }
        return found;
    }
    ...
    final class ElementInfo implements TypeInfo {
        ...
        @Override
        public boolean isPrimitive() {
            return element.asType().accept(new IsPrimitiveVisitor(), null);
        }
