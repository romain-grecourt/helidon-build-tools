...
import static io.helidon.build.util.Strings.padding;
...
public abstract class CommandModel extends CommandParameters {
     ...
     */
    public static String renderTable(Map<String, String> map) {
        int maxKeyWidth = map.keySet().stream().mapToInt(String::length).max().orElse(0);
        return map.entrySet().stream()
                  .map(e -> "  " + e.getKey() + padding(" ", maxKeyWidth, e.getKey()) + "    " + e.getValue())
