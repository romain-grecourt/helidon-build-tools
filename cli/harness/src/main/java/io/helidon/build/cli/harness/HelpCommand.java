...
class HelpCommand extends CommandModel {
    ...
    private final static class HelpCommandExecution implements CommandExecution {
        ...
        private void doExecute(CommandContext context, CommandModel model) {
            ...
            Log.info(String.format("%nUsage:\t%s %s [OPTIONS] %s%n", context.cliName(), model.command().name(), usage.toString()));
            Log.info(model.command().description());
            Log.info("\nOptions:");
            Log.info(renderTable(options));
        }
    }
}
